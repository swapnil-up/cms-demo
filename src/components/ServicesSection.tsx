    ...
    <section id="services" className="section section-alt">
      <div className="container">
        <div className="section-header">
          ...
          {section.subtitle && <p className="section-subtitle" data-tina-field="subtitle">{section.subtitle}</p>}
        </div>
        <div className={styles.grid}>
          {services.map((service, i) => (
            <div key={i} className={styles.card}>
              <div className={styles.icon}>{String(i + 1).padStart(2, "0")}</div>
              <h3 className={styles.title} data-tina-field="title">{service.title}</h3>
              {service.description && (
                <p className={styles.desc} data-tina-field="description">{service.description}</p>
              )}
            </div>
          ))}
        </div>
